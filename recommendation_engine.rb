...
sorted_most_rated_movies = most_rated_movies.sort_by { |_, v| v }.reverse
...
movie_data.each do |item|
  top_hundred_movies.each do |movie|
    movie << item[1] if item[0] == movie[0]
  end
end
...
print '(keep in mind that the more movies you review the more accurate our recommendations will be):'
...
while counter < number_of_movies_to_rate
  ...
  # Following lines of code get user responses and store them in a hash
  # while assigning the user with the userID '5000'
  ...
  # This pushes the users ratings into the movies_and_ratings array
  counter += 1
end
...
def create_hash_of_movieid_and_ratings_for_each_user(user_data_array, userid)
  ...
  user_data_array.each do |item|
    next if item[0] != userid

    # push movieId into an array
    all_movies_rated_by_each_user << item[1]
    # change the ratings to floats to use the pearson gem
    all_ratings_by_each_user << item[2].to_f
  end
...
counter = 1
array_of_user_hashes = []
## used the method create_hash_of_movieid_and_ratings_for_each_user in a loop
## to create hashes for all the users in the ratings.csv file
while counter <= number_of_users_in_db
  user = create_hash_of_movieid_and_ratings_for_each_user(user_data, counter.to_s)
...
array_of_id_of_user = []
while counter <= number_of_users_in_db
  array_of_id_of_user << counter.to_s
  counter += 1
end
...
# '5000' is the assigned userId to the user
recommended_list = []
recommended_movies.each do |movie_id|
  movie_data.each do |movie|
    recommended_list << movie[1].green if movie_id[0] == movie[0]
  end
end
